...
require_source 'NSEvent\.addLocalMonitorForEvents\(matching: \[\.keyDown, \.flagsChanged\]\)' 'hotkey recorder captures key and modifier double taps'
...
require_source '@State private var pendingDoubleTapKeyCode: UInt16\?' 'hotkey recorder tracks a pending first key for double-tap recording'
require_source 'pendingDoubleTapKeyCode == event\.keyCode' 'hotkey recorder accepts pressing the same key twice'
