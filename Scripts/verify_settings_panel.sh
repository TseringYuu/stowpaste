...
require_source 'Toggle\("", isOn: \$isOn\)' 'launch at login uses the native macOS toggle'
require_source '\.toggleStyle\(\.switch\)' 'launch at login follows the system switch appearance'
require_source '\.controlSize\(\.small\)' 'native launch at login switch remains compact'
